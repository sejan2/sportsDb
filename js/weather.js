...
const loadWeather = () => {
    ...
    searchInput.value = '';
    const url = `https://api.openweathermap.org/data/2.5/weather?q=${searchText}&appid=b7889962aa67c781ca66163cb1cc2442`
    fetch(url)
        .then(res => res.json())
...
const displayData = country => {
    ...
    <h1>${country.main.temp}</h1>
    <h3>${country.coord.lon}</h3>
    <h1 class="lead">${country.country}</h1>`
    weatherId.appendChild(div);
}
...
//     setTimeout(() => {
//         loader.style.display = 'none';
//         coord.innerHTML = `Latitude: ${currentCity.coord.lat}<br>Longitude: ${currentCity.coord.lon}`;

//         const country = currentCity.sys;
//         countryName.innerText = country.country;

//         const tempareture = currentCity.main
//         temp.innerHTML = `${((Number(tempareture.temp)) - 273).toFixed(2)} &deg;C`;

//         const weathers = currentCity.weather;
//         weathers.map(weather => {
//             condition.innerText = weather.main;
//             weatherImg.innerHTML = `<img src="https://openweathermap.org/img/wn/${weather.icon}@2x.png">`
//         });
